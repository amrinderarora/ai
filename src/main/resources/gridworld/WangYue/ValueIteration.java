...
public class ValueIteration{
    ...
    for (int i = 0; i<gw.n; i++){
      for (int j = 0; j<gw.m; j++){
        ...
        if (gw.gridWorld[i][j]=='W')
          ...
          int n = i*gw.m+j;
          temp="["+df.format(qVlaue.get(n).get(0))+", "+df.format(qVlaue.get(n).get(1))+", "+df.format(qVlaue.get(n).get(2))+", "+df.format(qVlaue.get(n).get(3))+"]";
        }
  ...
  static Vector<Vector<Double>> getQStar(GridWorld gw){
    Vector<Vector<Double>> qValue= new Vector<Vector<Double>>();
    initVector(qValue,gw);
    //loop until converge
    while (true){
      double delta = 0; //value change from last iteraton
      Vector<Vector<Double>> next = new Vector<Vector<Double>>();
      initVector(next,gw);
      for (int i = 0; i<gw.n; i++){
        for (int j = 0; j< gw.m; j++){
          int n = i*gw.m+j;
 ...
 static void printVector(Vector<Vector<Double>> V, int n, int m){
   ...
   for (int i = 0; i < n; i++){
     ...
     for (int j = 0; j < m; j++){
       int k = i*m+j;
       System.out.print(" [");
       System.out.print(df.format(V.get(k).get(0))+", "+df.format(V.get(k).get(1))+", "+df.format(V.get(k).get(2))+", "+df.format(V.get(k).get(3)));
       System.out.print("] ");
     }
   }
   System.out.println("\n****************");
